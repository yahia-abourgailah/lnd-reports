...
def _roster_users(session: Session) -> dict[str, User]:
    """Everyone `get_users` currently returns, keyed by `odoo_id`.

    The roster endpoint post-dates this transform's first design, which read
    people out of program payloads alone and said so — "KNOWN GAP: THIS IS NOT
    A ROSTER". This is the source that closes it, and with it Participation
    Rate and Coverage Gap: the denominator is 1,435 staff rather than the 419
    who happen to have attended something.

    The roster payload validates as `User` unchanged. It carries no position,
    email or mobile — those exist only inside programs — which is exactly why
    both sources are loaded rather than one replacing the other.

    A malformed row is skipped rather than failing the pass. The roster is a
    denominator, and one unreadable person should cost one person, not the
    whole figure.
    """
    users: dict[str, User] = {}
    for record in current(session, source=Source.CRM, entity=Entity.EMPLOYEE):
        try:
            user = User.model_validate(record.payload)
        except ValidationError:
            log.warning("roster_user_unreadable", extra={"source_id": record.source_id})
            continue
        if user.odoo_id:
            users[str(user.odoo_id)] = user
    return users
...
def transform_programs(session: Session, *, source_ids: list[str] | None = None) -> TransformResult:
    ...
    #
    # Two sources, not one. The roster is who works here; the `user` objects
    # nested in programs are who has ever trained, and the two do not agree —
    # 149 of the 419 trained people are not on the roster, being 126 leavers
    # and 23 from two companies that no longer exist separately.
    #
    # Programs are collected first and the roster second, so the roster wins
    # wherever both have an opinion. A program payload is a snapshot from
    # whenever that program ran; letting it win would move people back into
    # departments they have since left.
    users: dict[str, User] = {}
    for _, program in parsed:
        users.update(collect_users(program))

    roster = _roster_users(session)
    users.update(roster)

    employees = load_employees(session, users, roster_ids=set(roster) if roster else None)
