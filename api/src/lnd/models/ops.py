...
class AlertKind(StrEnum):
    ...
    #: Records have been excluded from figures for longer than anybody should
    #: have to notice on their own. The queue is visible in the console either
    #: way; this is what turns "somebody will look" into "somebody was told".
    EXCLUSIONS_AGEING = "exclusions_ageing"
    #: A period's report was generated and did not reach anybody. The one
    #: failure that is otherwise silent by construction: nothing on a screen
    #: changes when a mail does not arrive.
    REPORT_UNDELIVERED = "report_undelivered"
...
class ExportEdition(Base):
    ...
    # -- delivery -----------------------------------------------------------
    # Recorded on the edition rather than in a log, because "did the September
    # report actually go out?" is asked months later by somebody who does not
    # have the logs. Null on an edition somebody generated by hand: it was
    # downloaded, not sent, and pretending otherwise would make the trail claim
    # a delivery nobody made.
    delivered_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    #: Comma-separated, as sent. The distribution list is resolved at send time,
    #: so storing the addresses is the only way to answer "who got March"
    #: after somebody is added to the list in April.
    delivered_to: Mapped[str | None] = mapped_column(Text, nullable=True)
    #: Why it did not go. Present exactly when a send was attempted and failed,
    #: or was skipped for want of a host or a recipient — never a bare
    #: "failed", because "SMTPAuthenticationError" and "timed out" send
